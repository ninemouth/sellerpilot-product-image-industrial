...
44a. 当当前环境权限不足导致必要动作无法继续时，必须用用户能理解的能力名请求授权，例如“需要你授权我启动本地临时审核服务”“需要你授权我访问网络以检查更新”“需要你授权我同步已安装 skill”。不得说“sandbox 禁止”“受控权限重跑”“绕过沙箱”或粘贴 raw permission error。**这条不适用于已配置外部 provider 的参考图上传。** 当 resolver 已选择 `third_party_proxy` + `ready`，不得问“是否同意把参考图发给外部通道/是否继续”；该上传已由 profile 配置授权。若宿主/组织策略仍在请求前阻断外发，记录 `external_provider_host_policy_blocked`，保留 run，并说明需要由环境或组织允许同一已选 route；不得把它改写成用户二次授权，也不得用本地安全稿替代正式成品。
44b. 已在安装或更新阶段配置且 resolver 已返回 `third_party_proxy` + `ready` 的外部图片代理，视为已获一次性执行许可；这覆盖用户提供的参考图上传、日常生图、下载、重试、QA 和交付，任何步骤都不得再次向用户索要外部 provider 授权。若宿主无法连通该 provider，记录 `external_provider_transport_unavailable`；若宿主/租户策略在 provider 收到请求前拒绝外发，记录 `external_provider_host_policy_blocked`。两者都必须保留 run、不计为远端 provider 请求失败、不消费 retry 预算，并且不得改走原生/其他 provider 或产出冒充正式成品的确定性本地稿。
...
46. 共享 tldraw 服务的模板和依赖必须在 skill 安装或更新时预热到 `${CODEX_HOME:-$HOME/.codex}/sellerpilot-product-image-industrial/canvas-service`。若更新发现依赖缺失或 lockfile 已变化，必须按当前 lockfile 选择已检测到的 Node 包管理器完成锁定安装再结束更新；生图结束后的画布启动不得执行依赖安装，只能复用已准备依赖并做就绪检查。
