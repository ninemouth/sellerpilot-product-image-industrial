...
39a. 长耗时任务、provider 失败、调度乱序或性能优化前必须运行 `production-phase-tracer.mjs` 写入 `telemetry/phase-trace.json`。优化依据必须来自当前 run 的 phase/job 事实，例如 source_preflight、planning、provider_total、provider_first_byte、download、QA、export、canvas_ready 的 p50/p95 或具体 job span；不得把 `production-efficiency-plan` 中的预算数当作真实 SLA。
39b. `production-efficiency-plan` 中的 parallelizable groups 必须能落到 `production-orchestrator.mjs` 的 run-local DAG task file：独立的 source/platform/provider/brief 任务可并行，identity、physical truth、prompt、anchor QA 和 final delivery gate 才能作为汇合点。不得把并行表格当成调度完成证据；真实调度证据是 `orchestration/production-orchestrator-state.json`。
39c. 同一图片角色或同类场景 provider 失败达到 retry/circuit 阈值时，必须运行 `provider-instability-circuit-breaker.mjs`。若报告要求停止 provider retries，不得继续堆同类 prompt；只能复核已批准资产、降级不稳定场景、用合规衍生资产补位，或先让用户确认是否继续高成本重试。
39d. 最终 manifest 必须记录 final image lineage。任何 `derived_from_approved_generated_asset`、`repaired_final_asset`、`local_text_overlay` 或 `personalized_text_overlay` 都必须有来源、转换类型和对应 gate/contract 证据；不得把裁切/衍生/本地文字合成图冒充新的 provider 原生生成图。
39e. 历史 run 若已经有最终图但 manifest 缺少明确 lineage，必须先运行 `backfill-final-image-lineage.mjs`，再重跑 personalized text contract、export gate、lineage gate 和 final delivery gate；不得通过整套重生图来掩盖旧进度记录缺失。
39f. 全局 provider timeout、并发或 retry 默认值不得依据单个长耗时 run 调整。必须用 `provider-telemetry-summary.mjs` 聚合多个 `telemetry/phase-trace.json`；当报告为 `insufficient_sample` 时，只允许做 run-local 修复或继续采样，不得改全局 provider 参数。
39g. Revision repair 若复用上一 run 或已批准 provider 资产，必须运行 `record-asset-reuse.mjs` 写当前 run 的 `generated-assets/asset-reuse-manifest.json` 和 `progress-reused-*.json`；phase trace 必须把 `asset_reuse_ms`、`local_compositor_ms` 与真实 `provider_runtime_ms` 分开，不得把旧 provider summary 路径当作当前 run 的 provider 耗时。
39h. Final images 已落盘后必须给出用户可见状态：说明 final images 数量、正在执行 overview/tldraw/final gate、不会重生图。超过 15 分钟或 final export 后必须用 `runtime-watchdog.mjs --auto-close-ready`；若分类为 `ready_but_not_closed`，自动执行缺失的 overview/tldraw/final-delivery-gate 并收口，失败才汇报 blocked reason。
39i. 关键 run artifact 不得手工 patch 或夹带 markdown/patch 分隔符。`generation-progress.json`、`anchor-batch-qa-decision.json`、`final-images-manifest.json`、overview report 和 QA reports 必须通过脚本写入并通过 `production-artifact-integrity-gate.mjs`；本地 artifact 污染只能修复/重建对应 JSON，不得触发 provider 重生图。
39j. 所有有源商品身份证据或带 `legacy_fallback`、`derived`、`repaired`、`local_overlay` lineage 的 final images 必须通过 `identity-consistency-gate.mjs`。逐图对比源素材/identity lock，检查轮廓、材质、颜色、五金、手柄/肩带、开口、配件、微细节；`needs_visual_review`、缺少逐图 review、fallback 未明确 pass、或任何身份漂移都不得通过 Final Delivery Gate。
...
44b. 已在安装或更新阶段配置且 resolver 已返回 `third_party_proxy` + `ready` 的外部图片代理，视为已获一次性执行许可；日常生图、下载、重试、QA 和交付中不得再次向用户索要外部 provider 授权。若宿主无法连通该 provider，记录 `external_provider_transport_unavailable`，保留 run，并把修复归入安装/更新阶段；不得改走原生或其他 provider，也不得把它算作远端 provider 请求失败或消费重试预算。
...
47a. 对印花/提花/织物图案是商品身份的包袋、服装、配饰和家纺，必须按 surface material canonical 处理，不得只写“相似图案/类似纹理”prompt。女包/桶包类还必须逐图锁定并复核桶身比例、包口/内里、肩带/提手路径、底部/侧面弧度、图案尺度/网格和织物纤维质感；漂移成普通托特、硬挺皮包、亮面塑料感、丝绸感或自由重画图案时，identity consistency 和 final delivery 必须失败。
48. 对用户默认只展示并安装 `$sellerpilot-product-image-industrial`。`sellerpilot-product-image-industrial-thinkai` 和旧的 `sellerpilot-product-image-industrial-proxy` 仍仅保留为仓库内迁移模板；只有用户明确要求直接调用第三方代理时，才使用 `standalone/image-proxy` 这个独立 provider skill。它不随主 skill 默认安装，不承担主 skill 的 workflow/QA/画布逻辑，也不得被旧 compatibility alias 同步脚本覆盖。
49. 所有正式生成并进入当前 run `final-images` manifest 的图片，都必须在最终营销/导出/交付 gate 前执行 `adaptive-natural-image-finish-batch-all-generated-images`，不得只处理摄影图子集。该步骤是最终自动后处理环节，不得要求或建议用户把 FFT、profile、lineage、gate、噪点、模糊、锐化等内部机制写进 generation prompt；用户只需说“让这批图更自然”“降低 AI 味”“对这个 run 做自然质感收尾”或“独立测试自然质感能力”即可触发，复杂执行细节由 skill 内部决定。批处理必须结合 panel/role 元数据与像素特征，把每张图识别为 `photographic_scene`、`studio_product`、`macro_detail`、`graphic_text`、`transparent_asset` 或 `hybrid_commerce` 后使用对应克制参数；带可见文字的图必须保护/恢复文字区域并在处理后通过逐图哈希绑定的 `post-natural-finish-visible-text-review`，透明图必须保留原 alpha。允许记录 FFT 频谱诊断并只在检测到具体周期性条纹、网格或振铃伪影且超过 profile 阈值时做克制 notch attenuation；不得做通用 AI 频率指纹压制。原图必须备份到当前 run 的 `generated-assets/natural-finish-originals/`，整批 staging 全部成功后才可替换 final images，失败时不得留下部分处理结果。必须写 batch/gate/per-asset proof 与 `natural_image_finish` lineage，之后重新运行身份一致性、lineage、营销、导出、artifact integrity 和 Final Delivery Gate。该能力只用于降低过度平滑、统一塑料感和不自然数字纹理，不得宣称或优化为规避 AI 检测；不得加入 CLIP-based adversarial perturbation、torch 检测器对抗或任何针对 AI 检测器的不可见扰动。Python/OpenCV/NumPy/Pillow/SciPy/FFmpeg 依赖只能在 skill 安装或更新阶段自动检测和预热；正式生产阶段只做就绪检查，缺失时 blocked，不得临时改全局 Python 环境。
