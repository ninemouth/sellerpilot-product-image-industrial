...
14d. Always start final generation with `create-image-generation-dispatch.mjs`, which resolves the current provider mode into exactly one run-local execution route. `native_codex` produces the native handoff in 14c. `third_party_proxy` produces a third-party handoff and resolved runtime command; execute only that OpenAI-compatible runtime with `--run-dir` and `--role`, so it records the same provider budget/evidence ledger. Because this runtime opens an external provider connection and may download a generated asset, invoke it from the host's outbound-network-authorized execution mode on the first attempt (see `production-runtime-runbook.md`); do not first run it in a restricted shell. A user's image-generation request grants the selected execution route full authority for that run, but the host may still need to ask for its own network capability. If that capability is declined or unavailable, preserve the run and pause for authorization; do not silently retry native imagegen, Gamma, or any other provider. `configuration_required` is a technical configuration pause, not a request for renewed generation authorization.
