...
A configured third-party route authorizes its execution for the task. Treat provider selection plus the saved third-party configuration from skill installation/update as the one-time authorization boundary: when `third_party_proxy` resolves `ready`, execute that exact runtime directly and do not ask for another authorization during generation, asset download, retry, QA, or delivery. If the host cannot reach the provider, record `external_provider_transport_unavailable`; preserve the run, do not count a provider attempt, do not substitute another provider, and direct the repair to skill installation or update rather than prompting the user again.
...
14d. Always start final generation with `create-image-generation-dispatch.mjs`, which resolves the current provider mode into exactly one run-local execution route. `native_codex` produces the native handoff in 14c. `third_party_proxy` produces a third-party handoff and resolved runtime command; execute only that resolved runtime with `--run-dir` and `--role`, so it records the same provider budget/evidence ledger. Dispatch must pass `generation-spec/generation-spec.json.requested_size` unchanged as the default request size; provider capability metadata may not silently replace a platform target. ThinkAI uses the OpenAI-compatible `gpt-image-2` runtime by default; NVIDIA Build FLUX is an explicitly configured `nvidia_nim_flux` JSON-runtime route, never an automatic fallback. A ready configured third-party route is authorized during skill installation/update and must execute directly; do not ask for another authorization during the task. If the host cannot establish the connection, preserve the run and record `external_provider_transport_unavailable` as an installation/update setup block; do not count it as a remote provider request and do not silently retry native imagegen, Gamma, or any other provider. `configuration_required` remains a technical configuration pause, not a request for renewed generation authorization.
