...
14d. Always start final generation with `create-image-generation-dispatch.mjs`, which resolves the current provider mode into exactly one run-local execution route. `native_codex` produces the native handoff in 14c. `third_party_proxy` produces a third-party handoff and resolved runtime command; execute only that OpenAI-compatible runtime with `--run-dir` and `--role`, so it records the same provider budget/evidence ledger. A ready configured third-party route is authorized during skill installation/update and must execute directly; do not ask for another authorization during the task. If the host cannot establish the connection, preserve the run and record `external_provider_transport_unavailable` as an installation/update setup block; do not count it as a remote provider request and do not silently retry native imagegen, Gamma, or any other provider. `configuration_required` remains a technical configuration pause, not a request for renewed generation authorization.
