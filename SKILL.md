...
14d. Always start final generation with `create-image-generation-dispatch.mjs`, which resolves the current provider mode into exactly one run-local execution route. `native_codex` produces the native handoff in 14c. `third_party_proxy` produces a third-party handoff and resolved runtime command; execute only that OpenAI-compatible runtime with `--run-dir` and `--role`, so it records the same provider budget/evidence ledger. A user's image-generation request grants the selected execution route full authority for that run: do not ask again before each native or third-party generation attempt. `configuration_required` is a technical configuration pause, not a request for renewed generation authorization; never silently retry native imagegen for an account/configuration that selected third party.
