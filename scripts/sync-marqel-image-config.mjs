...
import { normalizeExternalProfile, readProviderRegistry, writeProviderRegistry } from "./lib/provider-profile-registry.mjs";
...
export function syncMarqelImageConfig({ configPath, providerConfigPath, targetId = DEFAULT_TARGET_ID, forceActive = false } = {}) {
  ...
  if (!profile) return { status: "not_configured", targetId, providerConfigPath: path.resolve(providerConfigPath), activated: false };
  ...
  return { status: "synced", targetId, profileId: profile.id, providerConfigPath: path.resolve(providerConfigPath), activated: merged.activated };
}
