...
function shellExamples(paths) {
  if (process.platform === "win32") {
    return {
      ...
      powershell_configure_image_provider: `cd "${paths.baseSkill}"; npm run configure:image-provider-interactive`,
    };
  }
  return {
    sh_codex_home: `export CODEX_HOME="${paths.codexHome}"`,
    sh_open_skills_dir: `open "${paths.skillsDir}" || xdg-open "${paths.skillsDir}"`,
    sh_configure_image_provider: `cd "${paths.baseSkill}" && npm run configure:image-provider-interactive`,
  };
}
