...
if (prompt.status !== 0) {
  console.error("Local image-provider key entry was cancelled or could not open.");
  process.exit(prompt.status || 1);
}
const key = parsePromptOutput(prompt.stdout);
if (!key) {
  console.error("No image-provider API key was entered; local configuration was unchanged.");
  process.exit(2);
}
...
if (configured.status !== 0) {
  console.error("Image-provider key was not saved to the local provider configuration.");
  process.exit(configured.status || 1);
}
const result = safeJson(configured.stdout) || {};
console.log(JSON.stringify({ status: "configured", provider_mode: result.provider_mode || "third_party_proxy", provider: result.provider || null, key_source: "macos_hidden_password_dialog", key_output: "never_printed" }, null, 2));
...
function openSecurePrompt() {
  if (process.platform === "darwin") {
    const dialog = 'display dialog "Paste your image API key. It updates only the local SellerPilot provider configuration and is never printed." default answer "" with hidden answer buttons {"Cancel", "Save"} default button "Save" with title "SellerPilot Image Provider Setup"';
...
function openWindowsPasswordDialog() {
  const script = [
    ...
    '$f=New-Object Windows.Forms.Form; $f.Text="SellerPilot Image Provider Setup"; $f.Width=460; $f.Height=165; $f.StartPosition="CenterScreen"; $f.Topmost=$true',
...
function openLinuxPasswordDialog() {
  const zenity = spawnSync("zenity", ["--password", "--title=SellerPilot Image Provider Setup", "--text=Paste your image API key. It stays in local provider configuration."], { encoding: "utf8" });
