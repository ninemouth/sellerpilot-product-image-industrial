...
if (prompt.status !== 0) {
  console.error("Local ThinkAI key entry was cancelled or could not open.");
  process.exit(prompt.status || 1);
}
const key = parsePromptOutput(prompt.stdout);
if (!key) {
  console.error("No ThinkAI image API key was entered; local configuration was unchanged.");
  process.exit(2);
}
...
if (configured.status !== 0) {
  console.error("ThinkAI key was not saved to the local provider configuration.");
