...
record("local processor-free runtime", () => {
  if (listFiles(path.join(skillRoot, "scripts"), (file) => file.endsWith(".py")).length) {
    throw new Error("The shipped production runtime must not contain local Python image processors.");
  }
  const pkg = readJson(path.join(skillRoot, "package.json"));
  for (const name of Object.keys(pkg.scripts || {})) {
    if (/^(prepare|check|finish):.*image/i.test(name)) {
      throw new Error(`package.json must not expose local image processor command ${name}.`);
    }
  }
});
...
record("tldraw shared service template sync dry run", () => {
  ...
  if (service.includes('spawnSync("npm", ["install"')) {
    throw new Error("runtime canvas startup must not use an unlocked npm install; preparation belongs to skill sync.");
  }
  for (const token of ["--prepare-only", "cleanInstallArgs", "resolvePackageManager", "blocked_canvas_dependencies_not_prepared", '"--strictPort"']) {
