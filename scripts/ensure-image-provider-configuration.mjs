...
for (const flag of ["config", "codex-config", "provider", "native-imagegen"]) if (args[flag]) resolveArgs.push(`--${flag}`, String(args[flag]));
...
function usage() { console.error("Usage: node scripts/ensure-image-provider-configuration.mjs [--config /abs/image-provider.json] [--codex-config /abs/config.toml] [--provider auto|native_codex|third_party_proxy] [--native-imagegen available|unavailable|unknown] [--no-prompt]"); process.exit(2); }
