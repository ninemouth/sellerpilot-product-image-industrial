...
const apiKey = String(args["api-key"] || stdinKey || process.env[apiKeyEnv] || process.env[defaultApiKeyEnv] || process.env[legacyApiKeyEnv] || "").trim();
if (!apiKey) { console.error(`Missing third-party image API key. Provide --api-key, --api-key-stdin, or set ${apiKeyEnv}.`); process.exit(2); }
const config = {
