...
After profile selection, the processor runs a visual-state camera/Photoshop realism check rather than a product-category recipe. It looks for image states such as `high_key_soft_product`, `low_key_moody_scene`, `flat_ai_render`, `glossy_reflective_surface`, `matte_or_smooth_surface`, `macro_texture_detail`, `graphic_text_layout`, `transparent_cutout_asset`, `lifestyle_camera_scene`, and `studio_clean_product`. These states come from visual metrics: very low edge density, compressed luminance variation, high/low key ratios, low-saturation palettes, warm highlight surfaces, weak high-frequency texture, large smooth regions, local-contrast readings, white-balance deviation, and scene/studio role metadata.
