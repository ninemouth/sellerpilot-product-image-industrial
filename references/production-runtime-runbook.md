...
`third_party_proxy` is an outbound HTTPS execution boundary. The runtime sends user-provided reference images and the generation/edit request to the resolved provider and, when the response contains a remote image URL, downloads that output as a second outbound request. Its permission is established by third-party provider setup during installation or update. Execute the resolved runtime directly for a ready route; do not add a task-level dialog asking whether reference images may be sent.

If the host cannot establish the connection, record `external_provider_transport_unavailable`. If an execution host or tenant policy blocks the outbound reference-image upload before the provider receives it, record `external_provider_host_policy_blocked`. Preserve the run and retry budget in either case. Do not retry blindly, consume provider retry budget, switch to Gamma/native imagegen/another provider, or create a deterministic local safe draft and present it as final. The latter is not a user-consent pause: the same selected route must be allowed by the environment or organization policy before retrying.
...
Before the runtime sends a provider request, it records one run-local ledger preflight. A local preflight block is not a remote provider failure and must not be diagnosed as a ThinkAI/NVIDIA outage. The runtime records a safe, specific code in `runtime/provider-failure-diagnostic-*.json`: `provider_evidence_delta_required`, `provider_run_budget_exhausted`, `provider_role_budget_exhausted`, `provider_run_state_invalid`, `provider_role_unregistered`, or `provider_ledger_preflight_failed`. These diagnostics also state whether the request was recorded and whether network execution started. Preserve completed assets; inspect the ledger rejection reason and repair only the run contract, role binding, budget, or upstream evidence that caused the block. Do not loosen the evidence-delta rule or switch providers merely to bypass it.

If the host cannot establish the provider connection, record `external_provider_transport_unavailable`. If the host/tenant rejects outbound reference-image transfer before the provider receives it, record `external_provider_host_policy_blocked`. Both mean no remote provider request was sent. Preserve the run and retry budget, do not substitute native generation or another provider, and never offer a local draft as an equivalent final result. A host-policy block is repaired by allowing the exact selected route in the environment or organization policy; it is not a user-authorization pause.
