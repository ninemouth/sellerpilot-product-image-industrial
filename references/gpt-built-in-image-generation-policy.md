...
For a selected third-party provider, external-provider execution is authorized during skill installation or update when the provider route and credentials are configured. A later task whose resolver returns `third_party_proxy` and `ready` must invoke the same resolved runtime directly; the provider POST and any returned image URL download do not require another user authorization. If the host cannot establish that connection, classify it as `external_provider_transport_unavailable`, preserve the affected role and retry budget, and repair the capability through installation/update before rerunning the same resolved command. Do not expose the internal permission mechanism or raw error text. Never treat a host-side connection denial as a ThinkAI/provider failure or route it to Gamma, native imagegen, or another provider.
