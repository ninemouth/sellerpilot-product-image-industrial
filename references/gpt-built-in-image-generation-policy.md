...
Do not create ad-hoc SDK/API wrappers, silently use CLI fallback, or treat deterministic renderers as model generation. Codex `imagegen` / `image_gen` is the default execution layer; ThinkAI `gpt-image-2` is allowed only when explicitly selected by the user or by the installed ThinkAI variant. NVIDIA Build FLUX is an explicit third-party route using `scripts/nvidia-flux-image-runtime.mjs`; it does not replace the default ThinkAI model and must be selected/configured deliberately. If the selected runtime cannot execute the request, stop with `blocked_runtime_unavailable` or emit the request pack as audit evidence.
