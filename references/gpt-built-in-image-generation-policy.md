...
Do not create ad-hoc SDK/API wrappers, silently use CLI fallback, or treat deterministic renderers as model generation. Codex `imagegen` / `image_gen` is the default execution layer in a new registry; ThinkAI `gpt-image-2` is allowed only through an explicitly selected external profile. NVIDIA Build FLUX is a built-in profile using `scripts/nvidia-flux-image-runtime.mjs`; it does not replace the currently selected profile and must be selected/configured deliberately. If the selected runtime cannot execute the request, stop with `blocked_runtime_unavailable` or emit the request pack as audit evidence.
...
If a necessary non-provider capability is permission-blocked, ask the user for authorization using the capability name before rerunning that step. This does not apply to reference upload on a ready configured third-party route, which must not trigger another consent prompt.

For a selected third-party provider, external-provider execution is authorized during skill installation or update when the provider route and credentials are configured. A later task whose resolver returns `third_party_proxy` and `ready` must invoke the same resolved runtime directly; uploading user-provided reference images, the provider POST, and any returned image URL download do not require another user authorization. Never ask “是否同意把参考图发到外部生成通道/是否继续？” for a ready route. If the host cannot establish that connection, classify it as `external_provider_transport_unavailable`. If the host or tenant blocks external reference-image upload before the provider receives it, classify it as `external_provider_host_policy_blocked`. Both preserve the affected role and retry budget; neither is a ThinkAI/provider failure, a user-consent pause, or a reason to route to Gamma, native imagegen, another provider, or a local deterministic draft presented as final. A host-policy block can only be repaired by allowing the same already-selected route in the environment or organization policy. Do not expose the internal permission mechanism or raw error text.

Before execution, resolve the platform-required ratio and provider request size with `scripts/resolve-generation-spec.mjs`. The resulting platform target size is passed unchanged by dispatch; an adapter must not silently choose a generic size. For source-backed roles, pass only the prepared references selected by `select-source-references.mjs`—normally one or two, lower when the runtime cap requires it—and enforce count/per-image/total byte limits again at dispatch. The export gate remains a final check. For multi-image runs, only approved anchor QA unlocks remaining concurrency of two.
