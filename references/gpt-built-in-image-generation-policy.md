...
For a selected third-party provider, request or use the host's outbound-network execution capability before the first runtime invocation: the provider POST and any returned image URL download both require it. If the selected runtime reports that this capability is not authorized, pause only the affected role and ask the user for authorization before rerunning the same resolved command. Use the user-facing capability name, such as local review service startup, network access for update checks, installed skill sync, or provider generation call. Do not expose the internal permission mechanism or raw error text. If authorization cannot be requested in the current session, mark only that step blocked, keep completed outputs, and tell the user which authorization is needed to continue. Never treat a host-side connection denial as a ThinkAI/provider failure or route it to Gamma, native imagegen, or another provider.
