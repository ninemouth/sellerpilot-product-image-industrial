...
## Case 51: printed fabric bucket bag identity and material drift
Expected: for a source product such as `[墨隐山城]绿色双喜单肩包` with green/red double-happiness cotton-poly outer fabric, brown cotton lining, 24 x 15 x 25 cm bucket body, 28 cm shoulder strap height, and hard washable bottom board, the run must create a canonical surface material lock and bag geometry/identity lock before final generation. A generated set fails if it turns the product into a generic tote, hard structured purse, leather/plastic/glossy bag, redraws the double-happiness motif as an approximate pattern, changes motif scale/grid, loses the brown lining, changes the strap route, or smooths away the woven fabric texture. `identity-consistency-gate.mjs` must require per-image pass evidence for bucket/body silhouette and proportions, opening/interior lining, strap/handle route, canonical motif/pattern, and woven/cotton-poly texture; a generic `pass` is not enough. `final-delivery-gate.mjs` must fail if source facts indicate printed/woven bag material but `surface-material/canonical-material-lock.json` or `surface-material-transfer-gate-report.json` is missing.
